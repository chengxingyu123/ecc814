...
SCA_SVAPI
ForestMap Slice_GetUnivData(const NodeData & inwhat, string & estr, bool & bret)
{
	ForestMap fmap;
	bret = GetUnivData(fmap, inwhat, estr);
	return fmap;
}

SCA_SVAPI
bool Slice_SubmitUnivData(const NodeData & inwhat, const ForestMap & fmap, ForestMap & ofmap, string  & estr, bool & bret)
{
	ForestMap fmaptemp;
	...
	while( mit != fmaptemp.end() )
	{
		...
		//WriteLog( szTemp );
		ofmap.insert(std::make_pair(mit->first.c_str(), nodedata));
...
SCA_SVAPI
ForestList Slice_GetForestData(const NodeData & inwhat, string & estr, bool & bret)
{
	ForestList fmap;
	bret = GetForestData(fmap, inwhat, estr);
	return fmap;
}

SCA_SVAPI
void SliceClearForestMap(ForestMap & fmap)
{
	fmap.clear();
}

SCA_SVAPI
void SliceClearForestList(ForestList & fmap)
{
	fmap.clear();
}
...
{
	WriteLog("--------GetUnivData Start--------");
	ForestMap::iterator pMap = fmap.begin();
	NodeData::const_iterator pNode;
	char strMsg[1024] = {0};
	fmap.clear();
	estr="";
	...
	//char szTemp[1024] = {0};
	//sprintf( szTemp, "dowhat=%s", what.c_str() );
	//WriteLog( szTemp );
	...
	{
		WriteLog("--------GetSvIniFileBySections Start--------");
		NodeData::const_iterator pNode = inwhat.begin();
		char strBuf[512] = {0};
		while(pNode != inwhat.end())
		{
			sprintf(strBuf , "%s=%s",pNode->first.c_str() , pNode->second.c_str());
			WriteLog(strBuf);
			pNode++;
		}
		...
		*/
		{
			sprintf(strBuf , "传入GetSvIniFileBySections的参数：filename=%s\tsections=%s" , filename.c_str() , sections.c_str());
			WriteLog(strBuf);
			if( GetSvIniFileBySections(fmap,filename,sections,"localhost",user) )
			{
				PutReturnForestMap( fmap,"return","return","true" );
				WriteLog("--------GetSvIniFileBySections End(true)--------");
				return true;
			}
			else
			{
				estr+= " Failed to GetSvIniFileBySections  ";
				PutReturnForestMap( fmap,"return","return","false" );
				WriteLog("--------GetSvIniFileBySections End(false)--------");
				return false;
			}
		}
		return true;
		...
		*/
		...
		try{
			...
			while(  std::getline(input_temp,tempkey)  )
			{
				...
				WriteLog( "删除节点的ID：" );
				WriteLog( tempkey.c_str() );

				//if( DelChildren(tempkey, boolauto) )
				// 删除的是监测器
				OBJECT monitorObj = Cache_GetMonitor( tempkey );
				if( monitorObj != INVALID_VALUE )
				{
					string strMonitorType = "";
					FindNodeValue( GetMonitorMainAttribNode(monitorObj), "sv_monitortype", strMonitorType );
				...
				// 删除的是设备
				OBJECT entityObj = Cache_GetEntity( tempkey );
				if( entityObj != INVALID_VALUE )
				{
					string strDeviceType = "";
					FindNodeValue( GetEntityMainAttribNode(entityObj), "sv_devicetype", strDeviceType );
					if( !strDeviceType.empty() )
					{
						if( DelDCMEntity( tempkey, estr ) )
						{
							delok = true;
						}
						else
	...
	else if(what.compare("TryToLogin_NoPassWord")==0)
	{
		bool ret(false),isTrial(true);;
		try
		{
			ret=TryToLogin_NoPassWord(fmap, inwhat, estr,isTrial);
			WriteLog( estr.c_str() );
		}
		catch(...)
		{
	...
	else if(what.compare("EntityCopy")==0)
	{
		...
		WriteLog( sourceId.c_str() );
		WriteLog( targetParentId.c_str() );
		...
		if( autoCreateTable.compare("false")==0 )
			...
			newid = CopyEntity( sourceId, targetParentId, estr );
		else
			//newid= EntityCopyAndCreateTable(sourceId, targetParentId);
			newid= CopyEntityAndCreateTable( sourceId, targetParentId, estr );
	...
	// 以下分支专用于url监测器的步数设置
	//------------------------------------------------
	else if(what.compare("StepUrlSetup")==0)
	{
		bool ret(false);
		try{
			ret= StepUrlSetup(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("EditUrlStep")==0)
	{
		bool ret(false);
		try{
			ret= EditUrlStep(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("AddUrlStep")==0)
	{
		bool ret(false);
		try{
			ret= AddUrlStep(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("SaveUrlStep")==0)
	{
		bool ret(false);
		try{
			ret= SaveUrlStep(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("DelUrlStep")==0)
	{
		bool ret(false);
		try{
			ret= DelUrlStep(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
...
//		MAPNODE ma;
		...
		else
		{
			if(!newone)
			{
				...
				if( (strdel.compare("_dcm") == 0) || (strdel.compare("_dcmmanagenode") == 0) )// 若编辑的设备是dcm设备，则编辑前先保留原来属性
				{
					EnumNodeAttrib( GetEntityMainAttribNode(obj), dcmPairList );
				}
			}
		...
		{
			//--------------------------------------------------------------
			// 增加内容：在svdb中创建和编辑dcm设备时，需在dcm服务器上同步
			// 增加人：邹晓
			// 增加时间：2009.03.26
			//--------------------------------------------------------------
			if( sv_devicetype.compare("_dcm") == 0 )
			{
				if( newone )// 创建设备
				{
					...
					{
						DelChildren( id );// 删除svdb中新创建的设备及其监测器，保持与dcm服务器同步
						PutReturnForestMap( fmap,"return","return","false" );
					...
					else
					{
						...
						sprintf( szTemp, "创建设备总耗时：%d", endTime-beginTime );
						WriteLog( szTemp );
						*/
						//-----------------------------------------------------------------------------------------
				...
				else// 编辑设备
				{
					...
					else
					{
						...
						endTime = ::GetTickCount();
						sprintf( szTemp, "编辑设备总耗时：%d", endTime-beginTime );
						WriteLog( szTemp );
						*/
						//-----------------------------------------------------------------------------------------
			...
			else if( sv_devicetype.compare("_dcmmanagenode") == 0 )
			{
				...
				else
				{
					...
					endTime = ::GetTickCount();
					sprintf( szTemp, "设置的dcm管理节点属性总耗时：%d", endTime-beginTime );
					WriteLog( szTemp );
					*/
					//-----------------------------------------------------------------------------------------
	...
	else if(what.compare("SubmitMonitor")==0)
	{
		...
		ForestMap::const_iterator pMap = fmap.begin();
		NodeData::const_iterator pNode;
		char strTempBuffer[1024] = {0};
		WriteLog("--------SubmitMonitor Start--------");
		WriteLog("fmap");
		while(pMap != fmap.end())
		{
			...
			{
				sprintf(strTempBuffer , "%s=%s" , pNode->first.c_str() , pNode->second.c_str());
				WriteLog(strTempBuffer);
				pNode++;
			}
			pMap++;
		}
...
//		MAPNODE ma;
	...
	// dcm主动报警
	//--------------------------------------------------------------
	else if(what.compare("addActiveAlert")==0)
	{
		bool ret(false);
		try{
			ret= AddActiveAlert(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in AddActiveAlert;   ";
		}
		//cxy 2012/10/08
		fmap.clear();
		if(ret)
	...
	else if(what.compare("handleActiveAlert")==0)
	{
		bool ret(false);
		try{
			ret= HandleActiveAlert(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("getActiveAlert")==0)
	{
		bool ret(false);
		try{
			ret= GetActiveAlert(fmap, inwhat, estr);	
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in GetActiveAlert;   ";
		}
...
/*为中组部开发的，主要用于将SQL中的数据写回SVDB*/
	else if(what.compare("CreateGroup_S") == 0)
	{
		bRet = CreateGroupInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
			strTemp = "false";
		PutReturnForestMap(fmap , "return" , "return" , strTemp);
		return bRet;

	}
	else if(what.compare("CreateEntity_S") == 0)
	{
		bRet = CreateEntityInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("CreateMonitor_S") == 0)
	{
		bRet = CreateMonitorInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("AddGroup_S") == 0)
	{
		bRet = AddGroupInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("AddEntity_S") == 0)
	{
		bRet = AddEntityInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("AddMonitor_S") == 0)
	{
		bRet = AddMonitorInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("RemoveGroup_S") == 0)
	{
		bRet = RemoveGroupInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("RemoveEntity_S") == 0)
	{
		bRet = RemoveEntityInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("RemoveMonitor_S") == 0)
	{
		bRet = RemoveMonitorInSVDB(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("RemoveEmptyGroup_S") == 0)
	{
		bRet = RemoveEmptyGroup(estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("RemoveEmptyEntity_S") == 0)
	{
		bRet = RemoveEmptyEntity(estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("RemoveEmptyMonitor_S") == 0)
	{
		bRet = RemoveEmptyMonitor(estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
	...
	else if(what.compare("GetMonitorReturnItem") == 0)
	{
		bRet = GetMonitorReturnItem(fmap , inwhat , estr);
		string strTemp;
		if(bRet)
			strTemp = "true";
		else
...
void PrintForestList(ForestList flist)
{
	ForestList::iterator pForest = flist.begin();
	NodeData::iterator  pNode;
	char strTemp[1024] = {0};
	while(pForest != flist.end())
	{
		pNode = pForest->begin();
...
bool GetForestData(ForestList & flist, const NodeData & inwhat, string & estr)
{
	...
	if(what.compare("GetTreeData")==0)
	{
		...
		ForestList::const_iterator mit = flist.begin();
		while( mit != flist.end() )
		{
...
/*
			WriteLog("GetForestData后获得数据头！");			
			PrintForestList(flist);				//打印出flist中的数据。
			WriteLog("GetForestData后获得数据尾！");
*/
	...
	else if(what.compare("QueryMassAlertLog")==0)
	{
		bool ret(false);
		try{
			ret= QueryMassAlertLog(flist, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
