...
{
	char szTemp[1024] = {0};
	WriteLog( "=============== LOGFILE_SSH_VALUE_IMPROVE =================" );
	...
	long	lNewTotalLine = 0, lOldTotalLine=0,lMatches = 0;
	char	szIniFile[256] = {0};
	BOOL	bError = FALSE;
	int nState;  //  for new logic   stop monitor as soon as error state
	std::string strDyn;
	...
	*/
	...
	if(SshLogFileLine(server, port, uid, pwd, szIniFile, logfile, privatekeyfile, lNewTotalLine))
	{
		sprintf(szReturn, "error=%s", FuncGetStringFromIDS("<%IDS_LOGFILE_01%>"));
		WriteLog( "SshLogFileSize error" );
		return FALSE;
	...
	DFN_GetPrivateProfileString(strmid, "size", "", buffer, sizeof(buffer), "logfile.ini");
	lOldTotalLine = atol(buffer);
	printf("oldTotalLine =%d\n",lOldTotalLine);
	

	if(lNewTotalLine == lOldTotalLine)
	{
		sprintf(szReturn, "totalline=%d$matches=%d$", 0, 0);	
	}
	else if(lNewTotalLine < lOldTotalLine)
	{
		sprintf(buffer, "%s", "logfile.ini");
		DFN_WritePrivateProfileString(strmid, "size", _ltoa(lNewTotalLine, tmp, 10) , buffer);
		sprintf(szReturn, "error=日志文件内容有被删除,请重新添加日志");
	...
	else
	{
		...
		if(append)
		{
			if(DFNParser_GetPrivateProfileString("tailLine", "command", "", buffer, sizeof(buffer), szIniFile) <= 0)
			{
				sprintf(szReturn, "error=%s", FuncGetStringFromIDS("<%IDS_LOGFILE_02%>"));
				bResult = FALSE;
				goto err;
		...
		{
			if(DFNParser_GetPrivateProfileString("headLine", "command", "", buffer, sizeof(buffer), szIniFile) <= 0)
			{
				sprintf(szReturn, "error=%s", FuncGetStringFromIDS("<%IDS_LOGFILE_02%>") );
				bResult = FALSE;
				goto err;
		...
		sprintf(buffer, "%d",lNewTotalLine-lOldTotalLine);
		strCmdInfo.Replace( "<addLineNum>", buffer );
		strCmdInfo.Replace( "<match>", match );
		strCmdInfo.Replace( "<file>", logfile );
		...
		GetColumnFromLine(strResult, 1, lMatches);
		
		sprintf(buffer, "%s", "logfile.ini");
		DFN_WritePrivateProfileString(strmid, "size", _ltoa(lNewTotalLine, tmp, 10) , buffer);

		sprintf(szReturn, "totalline=%ld$matches=%ld$", lNewTotalLine-lOldTotalLine, lMatches);
	}
	return true;
err:
	return bResult;
...
//dy---------------有的执行会出现这样多一条bash: shopt: login_shell: unknown shell option name\----------------
	int len1= -1;
	len1 = strResult.Find("bash: shopt: login_shell: unknown shell option name",0);
	if (len1!=-1)
	{
		int len2= strResult.GetLength();
		len1 = strlen("bash: shopt: login_shell: unknown shell option name");

		strResult = strResult.Right(len2-len1-1);
...
int SshLogFileLine(char *server, int port, char *uid, char *pwd, char *inifile, char *logfile,char *privatekeyfile, long &size)
{
	...
	if(DFNParser_GetPrivateProfileString("logfile", "totalline", "", cmd, sizeof(cmd), inifile) <= 0)
		return 1;
	...
	printf("len1 = %d\n",len1);
	//dy-------------------------------------------	
	printf("strResult = %s\n",strResult.GetBuffer(strResult.GetLength()));
	printf("sizeindex = %d\n",sizeindex);
	printf("size = %d\n",size);
	int  backNum = 10000;
	backNum = GetColumnFromLine(strResult, sizeindex, size);	
	printf("backNum = %d\n",backNum);
	return backNum;
}
...
int SshLogFileExist(char *server, int port, char *uid, char *pwd, char *inifile, char *logfile, char *privatekeyfile)
{
	...
	printf("strCmdInfo = %s\n",strCmdInfo.GetBuffer(strCmdInfo.GetLength()));
	printf("strResult = %s\n",strResult.GetBuffer(strResult.GetLength()));
	WriteLog("SshLogFileExist unix  return strResult:");
...
//dy---------------有的执行会出现这样多一条bash: shopt: login_shell: unknown shell option name\----------------
	...
	if (len1!=-1)
	{
		...
		strResult = strResult.Right(len2-len1-1);
		cout << "strResult = " << strResult << endl;
		WriteLog("SshLogFileExist change strResult:");
		WriteLog(strResult.GetBuffer(strResult.GetLength()));
	}
	printf("len1 = %d\n",len1);
//dy-------------------------------------------	
