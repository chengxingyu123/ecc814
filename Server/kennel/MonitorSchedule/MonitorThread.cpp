...
string MonitorThread::GetMonitorStatuString(int nEventType)
{
	string strType = "";
	switch(nEventType)
	{
...
//		printf("Return count :%d\n",m_RetValueCount);
	...
	if(Univ::enabltows)
	{
		...
		string strValue="";
		//LoadConfig lc;	
		bool bRet=m_Monitor->GetMonitorParaValueByName("sv_name",strValue);
		puts(strValue.c_str());
	    
		//MonitorId=1.9.32.2#LogTime=2012-07-24 11:07:24#MonitorName=Ping 192.168.9.131#MonitorStatus=ok#MonitorStr=包成功率(%)=100.00,数据往返时间(ms)=0.00,状态值(200表示成功 300表示出错)=200.00 
		strResult.Format("MonitorId=%s#LogTime=%s#MonitorName=%s#MonitorStatus=%s#MonitorStr=%s\n",m_Monitor->GetMonitorID(),time.Format().c_str(), strValue.c_str(),GetMonitorStatuString(nStatu).c_str(),(char *)m_strDisplay);
